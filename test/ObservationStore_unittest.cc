...
TEST_F(ObservationStoreTest, MaxSize) {
  ...
  EXPECT_EQ(store.observation_count(), (unsigned long)2);
}
...
TEST_F(ObservationStoreTest, AggregationFull) {
  ...
  // store should always be the specified size
  EXPECT_EQ(store.size(), (unsigned long)120);
    
  // store should have all observations
  EXPECT_EQ(store.observation_count(), (unsigned long)4);
